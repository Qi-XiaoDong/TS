...
Vue.directive('mymodel', {
  bind (el, binding, vnode) {
    ...
    el.value = value;
// 当触发input事件时，去更改数据的值（input 改变了，要传给组件）
  ...
  update (el, binding) {
    // 更改数据后，同步input的value值（组件的值改变了，要传input）
    const { value } = binding;
    el.value = value;
  },
})
...
// 设置newAge 的时候触发计算属性 的set方法
// 20
changeAge(){
  this.newAge -= 1   //  Hellow : this.newAge: 变化了 19
}
get () {
  return this.age + 1  // 19
}
// newAage ： 依赖于 app.vue的age
set (val) { // val = 19
  this.$listensens.onClick(val - 1)
}
// 改变age的值：age： 18
onClick(age){
  this.age = age;
